...
export function InstallPrompt() {
  ...
  return (
    <aside aria-label="Install app" className="fixed inset-x-3 bottom-[calc(5.75rem+env(safe-area-inset-bottom))] z-20 mx-auto max-w-md rounded-2xl border border-[#4F74C8]/25 bg-white/95 p-3 pr-11 text-sm text-[#20242F] shadow-lg backdrop-blur">
      <button aria-label="Dismiss install hint" className="absolute right-2 top-2 rounded-lg px-2 py-1 text-lg leading-none text-[#4A5365] focus-visible:outline-2 focus-visible:outline-[#4F74C8]" onClick={() => setDismissed(true)} type="button">×</button>
      {deferredPrompt ? (
        <>
          <p className="font-semibold">Install this dashboard for quicker access.</p>
          <button className="mt-2 rounded-lg bg-[#4F74C8] px-3 py-1.5 text-xs font-bold text-white" onClick={() => void install()} type="button">Install app</button>
        </>
      ) : (
