...
const inputClass = 'min-h-11 w-full rounded-xl border border-[#4F74C8]/30 bg-white px-3 text-[#20242F] outline-none transition focus:border-[#4F74C8] focus:ring-2 focus:ring-[#4F74C8]/25'
...
export function SettingsFeature({ adapter }: { adapter: StorageAdapter }) {
  ...
  if (!settings) return <section aria-busy="true"><h1 className="text-3xl font-black tracking-tight text-[#20242F]">Settings</h1><p className="mt-4 text-sm font-semibold text-[#4A5365]">Loading business settings…</p></section>
  ...
  return <section className="min-w-0 space-y-5" aria-labelledby="settings-heading">
    <header><p className="text-sm font-bold uppercase tracking-[0.16em] text-[#4F74C8]">Owner controls</p><h1 id="settings-heading" className="mt-1 text-3xl font-black tracking-tight text-[#20242F]">Settings</h1><p className="mt-2 text-sm leading-6 text-[#4A5365]">Menu prices are applied deterministically. Imported or AI-supplied prices are never trusted.</p></header>
    {message && <p role="status" className="flex gap-2 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-800"><Check aria-hidden="true" size={18} />{message}</p>}
    ...
    <button type="button" disabled={saving} onClick={() => void save()} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#4F74C8] px-4 font-bold text-white shadow-sm transition hover:bg-[#365aa8] disabled:cursor-not-allowed disabled:opacity-60"><Save aria-hidden="true" size={18} />{saving ? <><LoaderCircle aria-hidden="true" className="animate-spin" size={18} />Saving settings…</> : 'Save settings'}</button>
    <p className="flex gap-2 rounded-xl border border-[#4F74C8]/20 bg-[#FBF3D5]/60 p-3 text-sm text-[#4A5365]"><AlertTriangle aria-hidden="true" className="shrink-0 text-[#4F74C8]" size={18} />Pricing changes apply to new imports. Existing confirmed orders retain their recorded totals.</p>
  </section>
}
